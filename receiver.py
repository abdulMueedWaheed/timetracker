...
def load_script():
    try:
        subprocess.run(["qdbus", "org.kde.KWin", "/Scripting", "unloadScript", SCRIPT_NAME, ">/dev/null", "2>&1", "||", "true"])
        res = subprocess.run(["qdbus", "org.kde.KWin", "/Scripting", "loadScript", SCRIPT_PATH, SCRIPT_NAME], capture_output=True, text=True)
        script_id = res.stdout.strip()
        if script_id.isdigit():
            subprocess.run(["qdbus", "org.kde.KWin", f"/Scripting/Script{script_id}", "run", ">/dev/null"])
            print(f"Successfully loaded and hooked KWin script ID: {script_id}")

    except error as e:
        print(f"Failed to start KWin Script: {e}")
...
print("Listening for KWin active window transactions...")
try:
    GLib.MainLoop().run()
